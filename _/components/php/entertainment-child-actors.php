				<div class="panel-group" id="accordion">
					<div class="panel panel-default">
						<div class="panel-heading-business collapse2" id="collapse2">
							<h1 class="title1 panel-title">
								<a data-toggle="collapse" data-parent="#accordion" href="#collapseTwo" class="collapser">
									<div class="container">
										<h1 class="accords">Child Actors Program</h1>
									</div>
										<button type="button" class="btn mobile-toggle" data-toggle="button" data-target="#collapseTwo" id="btn-2"></button>
								</a>
								<div class="clearfix"></div>
							</h1>
						</div>	
																				
						<div id="collapseTwo" class="panel-collapse collapse in container">
							<div class="panel-body">
							
								<div class="content row">
									<div class="col-lg-12 col-md-12 ">
										<section class="five actors" id="actors">
											
												<section class="row">								
													<div class="col-lg-8 col-md-8 two-col-text">
														<h1 class="hidden-xs hidden-sm">Child Actors Program</h1>
														<h2>Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
Ut dignissim porttitor diam, ut dignissim metus hendrerit convallis. </h2>
														<img src="images/business-solutions/entertainment/child-actor-sm.jpg" alt="Kitchen" class="img-responsive pull-left">
														<p>Oakwood Toluca Hills is only minutes to most major studios, and are centrally located to dance studios, acting workshops and photographers. Our large, controlled-access complex features 1,100 apartments, on-site laundry and dry cleaners, a convenience store, swimming pools and playgrounds. In addition to the high-quality, secure housing, our Child Actor Program offers tutoring and educational programs to help 
parents learn the in's and out's of the entertainment business in a comfortable, safe place.</p>
														<p>We know how important it is to find a entertainment business. We know how important it is to find a comfortable, safe place for your child and family to stay - as quickly as possible. We'll locate housing and coordi- nate all the details, so that all your child has to worry about is learning his or her lines. </p>
														
										
													</div>
												</section><!--end row-->
												
												<section class="row details">
													<div class="col-lg-4 col-md-4">
														<h3>We take care of all the details</h3>
														<ul>
															<li>Coordinate move-in and move-out</li>
															<li>Offer consolidated billing</li>
															<li>Provide additional</li>
															<li>Household items</li>
															<li>Respond to your needs 24/7/365</li>
														</ul>
													</div>
												</section>
												
												<hr>
												
												<section class="row actor-resources">
													<div class="col-lg-4 col-md-4 media">
														<h3>Oakwood's Child Actor Program in the Media</h3>
														<ul class="news-thumbnails">
															<li class="clearfix">
																<img src="images/news/latest-news-tb-01.jpg" alt="latest-news-tb-01" class="pull-left">
																<p class="date">Oct 5, 2013</p>
																<p class="headline">Lorem Ipsum Dolore Sit Amet Oakwood</p>
															</li>
															<li class="clearfix">
																<img src="images/news/latest-news-tb-02.jpg" alt="latest-news-tb-01" class="pull-left">
																<p class="date">Oct 5, 2013</p>
																<p class="headline">Lorem Ipsum Dolore Sit Amet Oakwood</p>
															</li>
															<li class="clearfix">
																<img src="images/news/latest-news-tb-03.jpg" alt="latest-news-tb-01" class="pull-left">
																<p class="date">Oct 5, 2013</p>
																<p class="headline">Lorem Ipsum Dolore Sit Amet Oakwood</p>
															</li>
															<li class="clearfix">
																<img src="images/news/latest-news-tb-02.jpg" alt="latest-news-tb-01" class="pull-left">
																<p class="date">Oct 5, 2013</p>
																<p class="headline">Lorem Ipsum Dolore Sit Amet Oakwood</p>
															</li>
															<li class="clearfix">
																<img src="images/news/latest-news-tb-03.jpg" alt="latest-news-tb-01" class="pull-left">
																<p class="date">Oct 5, 2013</p>
																<p class="headline">Lorem Ipsum Dolore Sit Amet Oakwood</p>
															</li>
														</ul>
													</div><!--end one third-->
													
													<div class="col-lg-4 col-md-4 dates">
														<h3>Program Dates 2014</h3>
															<ul>
																<li>
																	<p>January to March 2014</p>
																	<a class="orange-link" href="#">Jan-March Rate Sheet &#8594;</a>
																</li>
																<li>
																	<p>July to August 2014</p>
																	<a class="orange-link" href="#">July-Aug Rate Sheet &#8594;</a>
																</li>
															</ul>
															<hr>
														<h3 class="car-rental">Enterprise Rent-A-Car at Oakwood Toluca Hills</h3>
														<img src="images/business-solutions/entertainment/enterprise.jpg" alt="enterprise" width="254" height="50" />
													
													</div><!--eng second third-->
													
													<div class="col-lg-4 col-md-4 resources">
														<h3>Acting Resources</h3>
															<ul>
																<li>
																	<p>Find agents, acting schools, photographers and more.<p>
																	<a class="orange-link" href="#">Informational Links &#8594;</a>
																	
																</li>
																<li>
																	<p>Learn About Oakwood’s Studio Tutoring Service<p>
																	<a class="orange-link" href="#">Tutoring Info &#8594;</a>
																	
																</li>
																<li>
																	<p>Helpful Books and Guides<p>
																	<a class="orange-link" href="#">Suggested Reading List &#8594;</a>
																	
																</li>
															</ul>
													</div>
													
													
												
												</section>
												
												<hr class="hidden-xs">
												
												<section class="row actors-faq">
													<div class="col-lg-8 col-md-8">
														<h3>Child Actors FAQ</h3>
														<div class="panel-group" id="accordion">
															<div class="panel panel-default">
																<div class="panel-heading">
																	<h4 class="panel-title">
																		<a data-toggle="collapse" data-parent="#accordion" href="#collapseSix">
          <div class="arrow-right"></div>What is the Child Actor Program?
		  																</a>
		  															</h4>
		  														</div>
		  														<div id="collapseSix" class="panel-collapse collapse in">
			  														<div class="panel-body">
        Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 3 wolf moon officia aute, non cupidatat skateboard dolor brunch. Food truck quinoa nesciunt laborum eiusmod. Brunch 3 wolf moon tempor, sunt aliqua put a bird on it squid single-origin coffee nulla assumenda shoreditch et. Nihil anim keffiyeh helvetica, craft beer labore wes anderson cred nesciunt sapiente ea proident. Ad vegan excepteur butcher vice lomo. Leggings occaecat craft beer farm-to-table, raw denim aesthetic synth nesciunt you probably haven't heard of them accusamus labore sustainable VHS.
																	</div>
																</div>
															</div>
															<div class="panel panel-default">
																<div class="panel-heading">
																	<h4 class="panel-title">
																		<a data-toggle="collapse" data-parent="#accordion" href="#collapseSeven">
           <div class="arrow-right"></div>How do I get my child an agent?
		  																</a>
		  															</h4>
		  														</div>
		  														<div id="collapseSeven" class="panel-collapse collapse in">
			  														<div class="panel-body">
        Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 3 wolf moon officia aute, non cupidatat skateboard dolor brunch. Food truck quinoa nesciunt laborum eiusmod. Brunch 3 wolf moon tempor, sunt aliqua put a bird on it squid single-origin coffee nulla assumenda shoreditch et. Nihil anim keffiyeh helvetica, craft beer labore wes anderson cred nesciunt sapiente ea proident. Ad vegan excepteur butcher vice lomo. Leggings occaecat craft beer farm-to-table, raw denim aesthetic synth nesciunt you probably haven't heard of them accusamus labore sustainable VHS.
																	</div>
																</div>
															</div>
															<div class="panel panel-default">
																<div class="panel-heading">
																	<h4 class="panel-title">
																		<a data-toggle="collapse" data-parent="#accordion" href="#collapseEight">
           <div class="arrow-right"></div>Why is Oakwood Toluca Hills so popular?
		  																</a>
		  															</h4>
		  														</div>
		  														<div id="collapseEight" class="panel-collapse collapse in">
			  														<div class="panel-body">
        Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 3 wolf moon officia aute, non cupidatat skateboard dolor brunch. Food truck quinoa nesciunt laborum eiusmod. Brunch 3 wolf moon tempor, sunt aliqua put a bird on it squid single-origin coffee nulla assumenda shoreditch et. Nihil anim keffiyeh helvetica, craft beer labore wes anderson cred nesciunt sapiente ea proident. Ad vegan excepteur butcher vice lomo. Leggings occaecat craft beer farm-to-table, raw denim aesthetic synth nesciunt you probably haven't heard of them accusamus labore sustainable VHS.
																	</div>
																</div>
															</div>
															
															<div class="panel panel-default">
																<div class="panel-heading">
																	<h4 class="panel-title">
																		<a data-toggle="collapse" data-parent="#accordion" href="#collapseNine">
          <div class="arrow-right"></div> Will I need a car in Los Angeles?
		  																</a>
		  															</h4>
		  														</div>
		  														<div id="collapseNine" class="panel-collapse collapse in">
			  														<div class="panel-body">
        Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 3 wolf moon officia aute, non cupidatat skateboard dolor brunch. Food truck quinoa nesciunt laborum eiusmod. Brunch 3 wolf moon tempor, sunt aliqua put a bird on it squid single-origin coffee nulla assumenda shoreditch et. Nihil anim keffiyeh helvetica, craft beer labore wes anderson cred nesciunt sapiente ea proident. Ad vegan excepteur butcher vice lomo. Leggings occaecat craft beer farm-to-table, raw denim aesthetic synth nesciunt you probably haven't heard of them accusamus labore sustainable VHS.
																	</div>
																</div>
															</div>
															<div class="panel panel-default">
																<div class="panel-heading">
																	<h4 class="panel-title">
																		<a data-toggle="collapse" data-parent="#accordion" href="#collapseTen">
           <div class="arrow-right"></div>Do other child actor families stay at Oakwood?
		  																</a>
		  															</h4>
		  														</div>
		  														<div id="collapseTen" class="panel-collapse collapse in">
			  														<div class="panel-body">
        Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 3 wolf moon officia aute, non cupidatat skateboard dolor brunch. Food truck quinoa nesciunt laborum eiusmod. Brunch 3 wolf moon tempor, sunt aliqua put a bird on it squid single-origin coffee nulla assumenda shoreditch et. Nihil anim keffiyeh helvetica, craft beer labore wes anderson cred nesciunt sapiente ea proident. Ad vegan excepteur butcher vice lomo. Leggings occaecat craft beer farm-to-table, raw denim aesthetic synth nesciunt you probably haven't heard of them accusamus labore sustainable VHS.
																	</div>
																</div>
															</div>
															<div class="panel panel-default">
																<div class="panel-heading">
																	<h4 class="panel-title">
																		<a data-toggle="collapse" data-parent="#accordion" href="#collapseEleven">
           <div class="arrow-right"></div>Have any famous child actors stayed at Oakwood?
		  																</a>
		  															</h4>
		  														</div>
		  														<div id="collapseEleven" class="panel-collapse collapse in">
			  														<div class="panel-body">
        Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 3 wolf moon officia aute, non cupidatat skateboard dolor brunch. Food truck quinoa nesciunt laborum eiusmod. Brunch 3 wolf moon tempor, sunt aliqua put a bird on it squid single-origin coffee nulla assumenda shoreditch et. Nihil anim keffiyeh helvetica, craft beer labore wes anderson cred nesciunt sapiente ea proident. Ad vegan excepteur butcher vice lomo. Leggings occaecat craft beer farm-to-table, raw denim aesthetic synth nesciunt you probably haven't heard of them accusamus labore sustainable VHS.
																	</div>
																</div>
															</div>
															
															
															<div class="panel panel-default">
																<div class="panel-heading">
																	<h4 class="panel-title">
																		<a data-toggle="collapse" data-parent="#accordion" href="#collapseTwelve">
           <div class="arrow-right"></div>Can my daughter bring her dog?
		  																</a>
		  															</h4>
		  														</div>
		  														<div id="collapseTwelve" class="panel-collapse collapse in">
			  														<div class="panel-body">
        Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 3 wolf moon officia aute, non cupidatat skateboard dolor brunch. Food truck quinoa nesciunt laborum eiusmod. Brunch 3 wolf moon tempor, sunt aliqua put a bird on it squid single-origin coffee nulla assumenda shoreditch et. Nihil anim keffiyeh helvetica, craft beer labore wes anderson cred nesciunt sapiente ea proident. Ad vegan excepteur butcher vice lomo. Leggings occaecat craft beer farm-to-table, raw denim aesthetic synth nesciunt you probably haven't heard of them accusamus labore sustainable VHS.
																	</div>
																</div>
															</div>
															<div class="panel panel-default">
																<div class="panel-heading">
																	<h4 class="panel-title">
																		<a data-toggle="collapse" data-parent="#accordion" href="#collapseThirteen">
           <div class="arrow-right"></div>Los Angeles is so big, will I feel safe at Oakwood?
		  																</a>
		  															</h4>
		  														</div>
		  														<div id="collapseThirteen" class="panel-collapse collapse in">
			  														<div class="panel-body">
        Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 3 wolf moon officia aute, non cupidatat skateboard dolor brunch. Food truck quinoa nesciunt laborum eiusmod. Brunch 3 wolf moon tempor, sunt aliqua put a bird on it squid single-origin coffee nulla assumenda shoreditch et. Nihil anim keffiyeh helvetica, craft beer labore wes anderson cred nesciunt sapiente ea proident. Ad vegan excepteur butcher vice lomo. Leggings occaecat craft beer farm-to-table, raw denim aesthetic synth nesciunt you probably haven't heard of them accusamus labore sustainable VHS.
																	</div>
																</div>
															</div>
															<div class="panel panel-default">
																<div class="panel-heading">
																	<h4 class="panel-title">
																		<a data-toggle="collapse" data-parent="#accordion" href="#collapseFourteen"> <div class="arrow-right"></div>Do child actors only come out for pilot season in January? How can my child attend school? 
		  																</a>
		  															</h4>
		  														</div>
		  														<div id="collapseFourteen" class="panel-collapse collapse in">
			  														<div class="panel-body">
        Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 3 wolf moon officia aute, non cupidatat skateboard dolor brunch. Food truck quinoa nesciunt laborum eiusmod. Brunch 3 wolf moon tempor, sunt aliqua put a bird on it squid single-origin coffee nulla assumenda shoreditch et. Nihil anim keffiyeh helvetica, craft beer labore wes anderson cred nesciunt sapiente ea proident. Ad vegan excepteur butcher vice lomo. Leggings occaecat craft beer farm-to-table, raw denim aesthetic synth nesciunt you probably haven't heard of them accusamus labore sustainable VHS.
																	</div>
																</div>
															</div>
															
															
															
														</div><!--accordion-->

													</div><!--col-->
												</section><!--actors-faq-->
												
										
										
										
										
										
									</section><!--five-->
								</div>
							</div><!--end content five-->
				
						</div><!--panel body-->
					</div><!--Collapse One-->
				</div><!--panel-->